...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        ...
        @if(Auth::id() !== $user->id)
            <form action="{{ route('users.destroy', $user->id) }}" method="POST" autocomplete="off" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun {{ $user->name }}?');" class="shrink-0 mt-2">
                @csrf
                @method('DELETE')
    ...
    <div class="glass-card p-6 sm:p-8 rounded-2xl shadow-sm border border-slate-200 bg-white">
        <form action="{{ route('users.update', $user->id) }}" method="POST" autocomplete="off" class="space-y-6">
            @csrf
            @method('PUT')
