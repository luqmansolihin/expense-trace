...
<div x-data="{ 
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="font-display text-2xl sm:text-3xl font-bold text-slate-900 leading-tight">Tambah Histori Hotel</h1>
            <p class="text-slate-600 text-xs sm:text-sm mt-1">Isi formulir reservasi hotel untuk mencatat histori transaksi dan invoice.</p>
        </div>
    </div>

    <!-- Form Container -->
    <div class="glass-card p-4 sm:p-8 rounded-2xl shadow-sm border border-slate-200 bg-white overflow-hidden">
        <form action="{{ route('hotels.store') }}" method="POST" enctype="multipart/form-data" autocomplete="off" class="space-y-6">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                    ...
                    <div>
                        ...
                        @if(!Auth::user()->isAdmin())
                            <div class="w-full glass-input rounded-xl px-4 py-2.5 text-sm bg-amber-50 flex items-center justify-between border border-amber-300">
