...
<div x-data="{ showModal: false }" class="max-w-4xl mx-auto space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <a href="{{ route('hotels.index') }}" class="text-xs font-semibold text-amber-600 hover:text-amber-700 flex items-center gap-1 mb-1">
                <i class="fa-solid fa-arrow-left"></i> Kembali ke Histori Hotel
            </a>
            <h1 class="text-lg sm:text-xl font-bold font-display tracking-tight text-slate-800 flex items-center gap-2">
                <i class="fa-solid fa-hotel text-amber-600"></i> {{ $hotel->hotel_name }}
            </h1>
            <p class="text-slate-500 text-xs sm:text-sm">Kode Booking: <span class="font-mono text-amber-600 font-semibold">{{ $hotel->booking_code ?: '-' }}</span> • Invoice: <span class="font-mono text-indigo-600 font-semibold">{{ $hotel->invoice_code }}</span></p>
        </div>

        <div class="flex items-center gap-2">
            <!-- Preview HTML & Stream PDF Button -->
            <button type="button" @click="showModal = true" class="px-4 py-2 rounded-xl text-xs font-semibold text-white bg-amber-600 hover:bg-amber-700 shadow-md shadow-amber-600/20 transition-all flex items-center gap-1.5">
                <i class="fa-solid fa-file-pdf"></i> Preview & Cetak Voucher Hotel
            </button>

            @can('update', $hotel)
                <a href="{{ route('hotels.edit', $hotel->id) }}" class="px-4 py-2 rounded-xl text-xs font-semibold text-amber-700 bg-amber-50 border border-amber-200 hover:bg-amber-600 hover:text-white transition-all flex items-center gap-1.5">
    ...
    <div class="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-sm space-y-5">
        <!-- Status & Dates Bar -->
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 rounded-xl bg-slate-50 border border-slate-200">
            <div>
                <div class="text-[10px] uppercase font-mono text-slate-500">Status Pembayaran</div>
            ...
            <div>
                <div class="text-[10px] uppercase font-mono text-slate-500">Tanggal Check-in</div>
                <div class="text-xs font-bold text-emerald-600 mt-1 font-mono">
                    {{ $hotel->check_in_date ? $hotel->check_in_date->format('d M Y') : '-' }}
                </div>
            </div>

            <div>
                <div class="text-[10px] uppercase font-mono text-slate-500">Tanggal Check-out</div>
                <div class="text-xs font-bold text-rose-600 mt-1 font-mono">
                    {{ $hotel->check_out_date ? $hotel->check_out_date->format('d M Y') : '-' }}
                </div>
            </div>

            <div>
                <div class="text-[10px] uppercase font-mono text-slate-500">Durasi Menginap</div>
                <div class="text-xs font-bold text-amber-700 mt-1 font-mono">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="p-4 rounded-xl bg-slate-50 border border-slate-200">
                <div class="text-[10px] uppercase font-mono text-slate-500 mb-1.5 flex items-center gap-1.5">
                    <i class="fa-solid fa-users text-amber-600"></i> Tamu yang Menginap ({{ $hotel->guest_count }} Orang)
                </div>
                <ul class="list-disc list-inside text-xs text-slate-800 font-medium space-y-1">
            ...
            <div class="p-4 rounded-xl bg-slate-50 border border-slate-200">
                <div class="text-[10px] uppercase font-mono text-slate-500 mb-1.5 flex items-center gap-1.5">
                    <i class="fa-solid fa-receipt text-emerald-600"></i> Total Biaya Reservasi
                </div>
                <div class="text-xl font-bold font-mono text-emerald-600 mt-1">
                    {{ $hotel->formatted_amount }}
                </div>
                <div class="text-[11px] text-slate-500 mt-1">
                    Booked by: <span class="text-indigo-700 font-semibold">{{ $hotel->booked_by }}</span>
                </div>
                <div class="text-[11px] text-slate-500">
                    Paid by: <span class="text-emerald-700 font-semibold">{{ $hotel->paid_by }}</span> @if($hotel->payment_date) ({{ $hotel->payment_date->format('d M Y') }}) @endif
                </div>
            </div>
        </div>
        ...
        @if($hotel->notes)
            <div class="p-4 rounded-xl bg-slate-50 border border-slate-200">
                <div class="text-[10px] uppercase font-mono text-slate-500 mb-1">Catatan</div>
                <p class="text-xs text-slate-700">{{ $hotel->notes }}</p>
            </div>
        @endif
        ...
        @if($hotel->attachment_path)
            <div class="p-4 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between">
                <div class="flex items-center space-x-2">
                    <i class="fa-solid fa-paperclip text-amber-600 text-sm"></i>
                    <span class="text-xs font-semibold text-slate-800">Lampiran Bukti Pembayaran / Invoice</span>
                </div>
                <a href="{{ asset('storage/' . $hotel->attachment_path) }}" target="_blank" class="px-3 py-1.5 rounded-xl bg-amber-100 text-amber-800 hover:bg-amber-600 hover:text-white border border-amber-200 text-xs font-semibold transition-all">
                    Buka Lampiran
                </a>
            </div>
        @endif
        ...
        <div class="pt-2">
            <h4 class="text-xs font-mono font-bold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-2">
                <i class="fa-solid fa-clock-rotate-left text-amber-600"></i> Log Aktivitas Perubahan Status
            </h4>
            <div class="space-y-2">
                @forelse($hotel->statusLogs as $log)
                    <div class="p-3 rounded-xl bg-slate-50 border border-slate-200 flex items-start justify-between gap-3 text-xs">
                        <div>
                            <div class="font-semibold text-slate-800 flex items-center gap-2">
                                <span class="text-amber-700">{{ $log->user_name }}</span>
                                <span class="text-[10px] px-2 py-0.5 rounded bg-slate-200 text-slate-600 font-mono capitalize">{{ $log->user_role ?? 'user' }}</span>
                            </div>
                            <p class="text-slate-600 text-[11px] mt-0.5">{{ $log->notes }}</p>
                        </div>
                        <div class="text-[10px] font-mono text-slate-400 shrink-0 text-right">
                    ...
                    <div class="p-6 space-y-5 bg-white text-xs">
                        ...
                        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="text-xs font-mono font-semibold text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200">{{ $hotel->guest_count }} Tamu ({{ $hotel->room_count }} Kamar)</span>
                            </div>
                            <div class="space-y-1">
                        ...
                        <div class="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-200">
                            <div>
                                ...
                                <span class="text-sm font-semibold font-mono text-amber-600 mt-0.5 block">{{ $hotel->booking_code ?: '-' }}</span>
                            </div>
                            <div>
                                <span class="text-xs text-slate-500 block">Kode Invoice</span>
                                <span class="text-sm font-semibold font-mono text-indigo-600 mt-0.5 block">{{ $hotel->invoice_code }}</span>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <span class="text-base font-bold text-emerald-600 font-mono mt-0.5 block">{{ $hotel->formatted_amount }}</span>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-200">
                            <div>
                                ...
                                <span class="text-xs font-semibold text-indigo-700 mt-0.5 block">{{ $hotel->booked_by }}</span>
                            </div>
                            <div>
                                <span class="text-xs text-slate-500 block">Pembayaran Oleh</span>
                                <span class="text-xs font-semibold text-emerald-700 mt-0.5 block">{{ $hotel->paid_by }}</span>
                            </div>
                            <div>
                                <span class="text-xs text-slate-500 block">Tanggal Booking</span>
                                <span class="text-xs font-semibold font-mono text-slate-700 mt-0.5 block">{{ $hotel->booking_date ? $hotel->booking_date->format('d M Y') : '-' }}</span>
                            </div>
                            <div>
                                <span class="text-xs text-slate-500 block">Tanggal Bayar</span>
                                <span class="text-xs font-semibold font-mono text-slate-700 mt-0.5 block">{{ $hotel->payment_date ? $hotel->payment_date->format('d M Y') : '-' }}</span>
                            </div>
                        </div>
                            ...
                            <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
                                ...
                                @foreach($hotel->statusLogs as $log)
                                    <div class="p-2.5 rounded-xl bg-white border border-slate-200 text-xs shadow-sm">
                                        ...
                                        <p class="text-slate-600 text-[11px] mt-0.5">{{ $log->notes }}</p>
                                        <div class="text-[10px] text-slate-400 font-mono mt-1">{{ $log->user_name }} ({{ ucfirst($log->user_role ?? 'user') }})</div>
                                    </div>
                                @endforeach
                            </div>
                        @endif
