...
@php
    ...
    $isBookerUnpaid = $isFinance && $hotel->status === 'Belum Bayar';
@endphp
...
<div x-data="{ 
    ...
    <div class="glass-card p-4 sm:p-8 rounded-2xl shadow-sm border border-slate-200 bg-white overflow-hidden">
        ...
        <form action="{{ route('hotels.update', $hotel->id) }}" method="POST" enctype="multipart/form-data" autocomplete="off" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                    ...
                    @if($isFinance)
                        <div class="md:col-span-2 bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-center gap-2.5 shadow-inner">
                            ...
                            <input type="hidden" name="paid_by" value="{{ $hotel->paid_by && $hotel->paid_by !== '-' ? $hotel->paid_by : Auth::user()->name }}">
