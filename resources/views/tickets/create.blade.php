...
<div class="max-w-4xl mx-auto min-w-0 w-full pb-12">
    ...
    <div class="glass-card p-4 sm:p-8 rounded-2xl shadow-sm border border-slate-200 bg-white overflow-hidden">
        <form action="{{ route('tickets.store') }}" method="POST" enctype="multipart/form-data" autocomplete="off" class="space-y-6">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="any" min="0" id="amount" name="amount" value="{{ old('amount') }}" placeholder="Contoh: 1500000" required class="w-full glass-input rounded-xl pl-10 pr-4 py-2.5 text-sm font-mono placeholder-slate-400 @error('amount') border-rose-500 @enderror">
                        </div>
                        @error('amount')
                            <p class="text-rose-600 text-xs mt-1">{{ $message }}</p>
                        @enderror
