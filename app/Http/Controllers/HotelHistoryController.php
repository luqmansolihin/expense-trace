    ...
    public function update(Request $request, HotelHistory $hotel)
    {
        ...
        if (!Auth::user()->isAdmin() && $hotel->status === 'Lunas') {
            $request->merge([
                ...
                'paid_by' => $request->input('paid_by', ($hotel->paid_by && $hotel->paid_by !== '-') ? $hotel->paid_by : '-'),
                'paid_by_user_id' => $request->input('paid_by_user_id', ($hotel->paid_by && $hotel->paid_by !== '-') ? $hotel->paid_by_user_id : null),
            ]);
        }
        ...
        if (!Auth::user()->isAdmin() && ($AuthUser = Auth::user()) && ($AuthUser->isBooker() || $AuthUser->isPayer())) {
            ...
            if ($hotel->status === 'Lunas') {
                ...
                if ($validated['status'] === 'Lunas') {
                    ...
                    $validated['status'] = 'Dibatalkan';
                    if ($hotel->status === 'Belum Bayar' || empty($hotel->payment_date)) {
                        $validated['paid_by'] = '-';
                        $validated['paid_by_user_id'] = null;
                        $validated['payment_date'] = null;
                    }
        ...
        if ($validated['status'] === 'Lunas') {
            ...
            if ($oldStatus === 'Belum Bayar' || empty($hotel->payment_date) || empty($validated['payment_date'])) {
                $validated['paid_by'] = '-';
                $validated['paid_by_user_id'] = null;
                $validated['payment_date'] = null;
            }
        ...
        $callback = function () use ($hotels) {
            ...
            foreach ($hotels as $h) {
                fputcsv($file, [
                    ...
                    $h->night_count,
                    $h->room_count,
                    implode(', ', $h->guests_list) ?: $h->guest_name,
                    $h->guest_count,
                    $h->booked_by,
                    $h->paid_by,
